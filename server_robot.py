...
@app.route('/send_move_command', methods=['POST'])
def send_move_command():
    ...
    if command:
        ...
        serial_control.send_command(serial_command)
        print(f"WYWOŁANO: Komenda ruchu: {command}")
        return jsonify({"status": "success", "message": f"Wysłano komendę ruchu: {command}"}), 200
...
def control_lidar():
    ...
    if serial_command:
        serial_control.send_command(serial_command)
        return jsonify({"status": "success", "message": f"Wysłano komendę Lidaru: {action}"}), 200

    return jsonify({"status": "error", "message": "Nieznana akcja Lidaru"}), 400
...
@app.route('/')
def index():
    return render_template('strona_robot.html')
